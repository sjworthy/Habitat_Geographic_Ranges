# Calculating minimum and maximum distance for each point to all other points
# Calculating 0.025 and 0.975 distance for each point to all other points
...
for(species.name in names(species.list)){
  ...
  species.data$min_microclim_distance <- min_dist_micro
  species.data$max_microclim_distance <- max_dist_micro
  species.data$min_topo_distance <- min_dist_topo
  species.data$max_topo_distance <- max_dist_topo
  species.data$min_soil_distance <- min_dist_soil
  species.data$max_soil_distance <- max_dist_soil
  
  safe_name <- gsub(" ", "_", species.name) 
  
  write.csv(
    species.data,
    file = paste0("./Distance.Results/", safe_name, "_gower_distances.csv"),
    row.names = FALSE)
...
for(species.name in names(species.list)){
  ...
  for(i in seq_len(n)){
    microclim.di = microclim.dist[i,] # distances from species i to all others
    microclim.di.2 = microclim.di[microclim.di > 0] # drop self-distance (0)
    q_micro = quantile(microclim.di.2, probs = c(0.025, 0.975), na.rm = TRUE)
    min_dist_micro[i] = q_micro[1]
    max_dist_micro[i] = q_micro[2]
    
    topo.di = topo.dist[i,] # distances from species i to all others
    topo.di.2 = topo.di[topo.di > 0] # drop self-distance (0)
    q_topo = quantile(topo.di.2, probs = c(0.025, 0.975), na.rm = TRUE)
    min_dist_topo[i] = q_topo[1]
    max_dist_topo[i] = q_topo[2]
    
    soil.di = soil.dist[i,] # distances from species i to all others
    soil.di.2 = soil.di[soil.di > 0] # drop self-distance (0)
    q_soil = quantile(soil.di.2, probs = c(0.025, 0.975), na.rm = TRUE)
    min_dist_soil[i] = q_soil[1]
    max_dist_soil[i] = q_soil[2]
  }
  
  species.data$min_q_microclim_distance <- min_dist_micro
  species.data$max_q_microclim_distance <- max_dist_micro
  species.data$min_q_topo_distance <- min_dist_topo
  species.data$max_q_topo_distance <- max_dist_topo
  species.data$min_q_soil_distance <- min_dist_soil
  species.data$max_q_soil_distance <- max_dist_soil
  
  safe_name <- gsub(" ", "_", species.name) 
  
  write.csv(
    species.data,
    file = paste0("./Quantile.Results/", safe_name, "_gower_quant_dist.csv"),
    row.names = FALSE)
...
# represents how atypical an individual is relative to the “average” of the species
...
for(species.name in names(species.list)){
  ...
  microclim.centroid = dist_to_centroids(microclim.dist,species.data$species)
  topo.centroid = dist_to_centroids(topo.dist,species.data$species)
  soil.centroid = dist_to_centroids(soil.dist,species.data$species)

  species.data$microclim.centroid = microclim.centroid$CentroidDistance
  species.data$topo.centroid = topo.centroid$CentroidDistance
  species.data$soil.centroid = soil.centroid$CentroidDistance
  
  safe_name <- gsub(" ", "_", species.name) 
  
  write.csv(
    species.data,
    file = paste0("./Centroid.Results/", safe_name, "_centroid_distances.csv"),
    row.names = FALSE)
}
