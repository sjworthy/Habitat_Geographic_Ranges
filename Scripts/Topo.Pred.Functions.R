  ...
  northeast <- function(terr){
    ...
    projection(slope_layer) <- projection(et)
    projection(aspect_layer) <- projection(et)
  ...
  mtpi <- function(dem){
    message("... computing etational position ...")
    fun <- function(x){
      x <- na.omit(x)
      sum(x) / length(x)
    }
  ...
  microclimate <- function(md, d, deltas, macro){
    ...
    d <- left_join(d, dplyr::select(dd, id, h:m), by = "id") %>%
      dplyr::select(h:m) %>%
      as.matrix()
    topo <- macro[[1:3]]
    topo[] <- d
