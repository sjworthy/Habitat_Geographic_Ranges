...
bioclimate <- function(dem, include_inputs = FALSE){
  ...
  projection(terr) = projection(dem)
  gc()
  ne <- northeast(terr)
  gc()
  wind <- windex(ne, terr)
...
raster_files <- list.files("./job1/", pattern = "*.tif", full.names = TRUE) # direct toward particular folder
...
clim = bioclimate(elev)

# Define output file name based on raster file name
output_file <- paste0("./job1.output/microclim_", output.name)
